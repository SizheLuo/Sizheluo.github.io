...
![](https://s3.uuu.ovh/imgs/2023/10/08/6cba2492f16c10d9.png)
	...
	public:
	    ...
	    void backTracking(vector<int>& candidates, int target, int start) {
	        ...
	        for (int i = start; i < candidates.size(); i++) {
	            if (i > 0 && candidates[i - 1] == candidates[i] && isUsed[i - 1] == false) {
	                continue;
	            }
	    ...
	    vector<vector<int>> combinationSum2(vector<int>& candidates, int target) {
	        ...
	        isUsed.resize(candidates.size(), false);
...
![](https://s3.uuu.ovh/imgs/2023/10/09/afe1c954223c4cac.png)
...
![](https://s3.uuu.ovh/imgs/2023/10/26/38617c5bd9d3113e.png)
	...
	public:
	    int wiggleMaxLength(vector<int>& nums) {
	        ...
	        for (int i = 1; i < nums.size(); i++) {
	            sequence.emplace_back(nums[i] - nums[i - 1]);
	        }
	
	        int result = 1;
	        int num;
	        for (int i = 0; i < sequence.size(); i++) {
	            if (result == 1 && sequence[i] != 0) {
	                result++;
	                num = sequence[i];
	            } else {
	                if (sequence[i] * num < 0) {
...
![](https://s3.uuu.ovh/imgs/2023/11/01/0464f0cdee69d576.png)
...
![](https://s3.uuu.ovh/imgs/2023/11/07/360208ba0235cb97.png)
...
![](https://s3.uuu.ovh/imgs/2023/11/08/f7243f796fadb446.png)
	...
	public:
	    bool canJump(vector<int>& nums) {
	        ...
	        while (left < nums.size() - 1) {
	            int preLeft = left;
	            int position = 0;
	            for (int i = preLeft + nums[left]; i > preLeft; i--) {
	                if (i >= nums.size() - 1) {
	                    return true;
	                }
...
![](https://s3.uuu.ovh/imgs/2023/11/08/966884e5f65ca1cb.png)
...
![](https://s3.uuu.ovh/imgs/2023/11/09/2c0ef11f836b0df6.png)
...
#### <font color="#0000dd">134. 加油站</font><br /> ####

![](https://s3.uuu.ovh/imgs/2023/11/11/bd6287508342d396.png)
...
![](https://s3.uuu.ovh/imgs/2023/11/19/011d3d59781ffe49.png)
...
![](https://s3.uuu.ovh/imgs/2023/11/20/93506216859d89bd.png)
...
![](https://s3.uuu.ovh/imgs/2023/11/24/385dca3b8e821430.png)
...
![](https://s3.uuu.ovh/imgs/2023/10/15/698ae0e7314a5efb.png)
...
![](https://s3.uuu.ovh/imgs/2023/10/15/e01389e2c315298e.png)
...
![](https://s3.uuu.ovh/imgs/2023/10/15/a5ac7239465e7115.png)
...
![](https://s3.uuu.ovh/imgs/2023/10/15/626492fcab35f315.png)
...
![](https://s3.uuu.ovh/imgs/2023/10/15/427cf5b91fd8618e.png)
...
![](https://s3.uuu.ovh/imgs/2023/10/17/2ea0f9d876f77cb5.png)
...
![](https://s3.uuu.ovh/imgs/2023/10/12/30fb62161a9d0a4b.png)
	    ...
	    vector<int> dailyTemperatures(vector<int>& temperatures) {
	        ...
	        for (int i = 0; i < temperatures.size(); i++) {
	            if (stk.empty()) {
	                ...
	                while (temperatures[curIndex] < temperatures[i]) {
...
![](https://s3.uuu.ovh/imgs/2023/09/26/de51109e9ae4bb1e.png)
	...
	public:
	    ...
	    void backTracking(vector<vector<char>>& grid, int row, int column) {
	        if (row < 0 || column < 0 || row >= grid.size() || column >= grid[0].size() || isUsed[row][column] == 1 || grid[row][column] == '0') {
	            return;
	        }
	
	        isUsed[row][column] = 1;
	        backTracking(grid, row - 1, column);
	        backTracking(grid, row + 1, column);
	        backTracking(grid, row, column - 1);
	        backTracking(grid, row, column + 1);
	    }
	
	    int numIslands(vector<vector<char>>& grid) {
	        result = 0;
	        isUsed.resize(grid.size(), vector<int>(grid[0].size(), 0));
	        for (int i = 0; i < grid.size(); i++) {
	            for (int j = 0; j < grid[0].size(); j++) {
	                if (isUsed[i][j] == 0 && grid[i][j] == '1') {
	                    result++;
...
![](https://s3.uuu.ovh/imgs/2023/10/16/b6e251e418514f99.png)

	class Solution {
	public:
	    vector<vector<int>> isUsed;
	    int result;
	    void dfs(vector<vector<int>>& grid, int row, int column, bool isFlight) {
	        if (row < 0 || row >= grid.size() || column < 0 || column >= grid[0].size() || isUsed[row][column] == 1 || grid[row][column] == 0) {
	            return;
	        }
	        ...
	        isUsed[row][column] = 1;
	        dfs(grid, row - 1, column, isFlight);
	        dfs(grid, row + 1, column, isFlight);
	        dfs(grid, row, column + 1, isFlight);
	        dfs(grid, row, column - 1, isFlight);
	    }
	    ...
	    int numEnclaves(vector<vector<int>>& grid) {
	        ...
	        for (int i = 0; i < grid.size(); i++) {
	            dfs(grid, i, 0, false);
	            dfs(grid, i, grid[0].size() - 1, false);
	        }
	
	        for (int i = 0; i < grid[0].size(); i++) {
	            dfs(grid, 0, i, false);
	            dfs(grid, grid.size() - 1, i, false);
	        }
...
![](https://s3.uuu.ovh/imgs/2023/10/21/83adecf7479cd444.png)
	...
	public:
	    ...
	    void dfs(vector<vector<char>>& board, int row, int column) {
	        if (row < 0 || column < 0 || row >= board.size() || column >= board[0].size() || isUsed[row][column] == 1 || board[row][column] == 'X') {
	            return;
	        }
	
	        path.emplace_back(vector<int>{row, column});
	        isUsed[row][column] = 1;
	    ...
	    void solve(vector<vector<char>>& board) {
	        ...
	        isUsed.resize(board.size(), vector<int>(board[0].size(), 0));
	        isNeedPadding = true;
	        path.clear();
...
![](https://s3.uuu.ovh/imgs/2023/10/22/3d5a9a29d5e80203.png)
	...
	public:
	    ...
	    bool isPacific;
	    bool isAtlantic;
	    void dfs(vector<vector<int>>& heights, int row, int column, vector<vector<int>>& isUsed) {
	        if (row < 0 || column < 0 || row >= heights.size() || column >= heights[0].size() || isUsed[row][column] == 1 || (isPacific && isAtlantic)) {
	            return;
	        }
	        ...
	        if (row > 0 && heights[row][column] >= heights[row - 1][column]) {
	            dfs(heights, row - 1, column, isUsed);
	        }
	
	        if (row < heights.size() - 1 && heights[row][column] >= heights[row + 1][column]) {
	            dfs(heights, row + 1, column, isUsed);
	        }
	
	        if (column < heights[0].size() - 1 && heights[row][column] >= heights[row][column + 1]) {
	            dfs(heights, row, column + 1, isUsed);
	        }
	    ...
	    vector<vector<int>> pacificAtlantic(vector<vector<int>>& heights) {
	        ...
	        for (int i = 0; i < heights.size(); i++) {
	            for (int j = 0; j < heights[0].size(); j++) {
	                isUsed.clear();
	                isUsed.resize(heights.size(), vector<int>(heights[0].size(), 0));
	                isPacific = false;
	                isAtlantic = false;
	                dfs(heights, i, j, isUsed);
